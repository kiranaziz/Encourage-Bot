...
# c.execute("INSERT INTO encouragements VALUES ('You are very cool!')")
# conn.commit()

# c.execute("INSERT INTO encouragements VALUES ('You are special!')")
# conn.commit()
...
def update_encouragements(encouraging_message):
  c.execute("SELECT * FROM encouragements WHERE encouragements=:message", {'message': encouraging_message})
  result = c.fetchone()
  conn.commit()
  
  if result is None:
    c.execute("INSERT INTO encouragements VALUES (:message)", {'message': encouraging_message})
    conn.commit()

def delete_encouragement(encouraging_message):
  c.execute("DELETE from encouragements WHERE encouragements=:message", {'message': encouraging_message})
  conn.commit()
...
async def on_message(message):
    ...
    if msg.startswith("$del"):
      ...
      if result:
        ...
        delete_encouragement(encouraging_message)
        c.execute("SELECT * FROM encouragements")
        result2 = c.fetchall()
        conn.commit()
        encouragements = result2
    ...
    if msg.startswith("$list"):
      ...
      encouragements = []
      if result:
        encouragements = result
      await message.channel.send(encouragements)
    
    #format of message = $responding false/true
